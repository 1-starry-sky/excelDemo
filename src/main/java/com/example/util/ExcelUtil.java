...
public class ExcelUtil {
    ...
    private static <T> List<ExcelClassField> getExcelClassFieldList(Class<T> clazz) {
        ...
        boolean hasExportAnnotation = false;   //是否为注解字段
        ...
        for (Field field : fields) {
            ...
            int sort = cf.getSort();
            //判断是否存在这个sort键，sort 可能为null或者数字，如果为数字则进入else排序，否则依次添加
            if (map.containsKey(sort)) {
                map.get(sort).add(cf);//
            } else {
                //通过sort 进行排序，sortList记录排序信息
        ...
        Collections.sort(sortList);//此处排序sortList
        // 获取表头
    ...
    /**
     * 获取注解情况，并映射字段（导出映射，格式如：0-未知;1-男;2-女）
     * @param field
     * @return
     */
    private static ExcelClassField getExcelClassField(Field field) {
        ...
        //转换映射关系
        cf.setKvMap(getKvMap(kv));   //--> （"0":”未知“,"1":"男"）
        return cf;
    }
    ...
    private static LinkedHashMap<String, String> getKvMap(String kv) {
        ...
        if (kv.isEmpty()) {
            return kvMap;//kvMap=null
        }
        //（0-未知;1-男;2-女）-->[0-未知,1-男,2-女]
        String[] kvs = kv.split(";");
        if (kvs.length == 0) {
            return kvMap;
        }
        for (String each : kvs) {
            ...
            //如何不是键值对形式则跳出该次循环，后面程序不执行，进入下一次循环
            if (eachKv.length != 2) {
                continue;
            }
            String k = eachKv[0];
    ...
    private static void export(HttpServletResponse response, File file, String fileName,
                               Map<String, List<List<Object>>> sheetMap, Map<Integer, List<String>> selectMap) throws IOException {
        // 整个 Excel 表格 book 对象
            ...
            for (int i = 0; i < sheetDataList.size(); i++) {
                ...
                for (int j = 0; j < rowList.size(); j++) {
                    ...
                    if (o instanceof URL) {
                        ...
                        setCellPicture(book, row, patriarch, i, j, (URL) o,"url",null);
                    }
                    else  if (o instanceof  InputStream){
                        // 如果要导出图片的话, 链接需要传递 URL 对象
                        setCellPicture(book, row, patriarch, i, j, null ,"local",(InputStream) o);
                    }
                    else {
     ...
     * @throws IOException
     */
    private static void setCellPicture(SXSSFWorkbook wb, Row sr, Drawing<?> patriarch, int x, int y, URL url,String imageType,InputStream is ) throws IOException {
        // 设置图片宽高
        ...
        //把读进来的图片放到一个ByteArrayOutputStream中，以便产生ByteArray
        try ( ByteArrayOutputStream outputStream = new ByteArrayOutputStream()) {
            ...
            // 设置图片位置
            ////col1 图片的左上角放在第几个列cell , row1 图片的左上角放在第几个行cell，// col2 图片的右下角放在第几个列cell，
    ...
    /**
     * 判断是否为数字，不是数字返回false
     * 缺点更新-如何为空字符串的话返回false
     * @param str
     * @return
     */
    private static boolean isNumeric(String str) {
        //nonNull-不为null返回ture
